...
func rescaleAndDraw(noise []float32, min, max float32, gradient []colour, pixels []byte) {
	scale := 255.0 / (max - min)
	offset := min * scale
...
func makeNoise(pixels []byte, frequency, lacunarity, gain float32, octaves int) {
	...
	// gradient := getDualGradient(colour{0, 0, 175}, colour{80, 160, 244}, colour{12, 192, 75}, colour{255, 255, 255})
	gradient := getGradient(colour{255, 0, 0}, colour{255, 242, 0})
